...
- **`formulario` (Sprint 1)** - já chega com parâmetros estruturados e validados pela API; entra direto no **nó 4** (geração de código), pulando os nós 1 a 3. Submissão de formulário não passa pela etapa intermediária de confirmação do usuário - o checkpoint do nó 3 só faz sentido para lidar com transcrição livre e ambígua ou extração inferida.
...
| `INSERT` e `SELECT` | Artefatos que o codegen produz: versão da regra extraída, código gerado, prompt enviado, resposta do modelo, explicação. O `SELECT` acompanha o `INSERT` porque o id nasce de `DEFAULT uuidv7()` no servidor e é lido de volta no próprio comando |
| `SELECT` | Transcrição do job e resultado da execução - o que precisam para trabalhar e retomar o grafo |
| **Nenhuma** | `job`, transições de estado, trilha de auditoria, histórico, fila de saída - tudo que representa estado e decisão de negócio |

Nenhum `UPDATE` e nenhum `DELETE` em lugar nenhum: a versão da regra que o codegen insere é imutável a partir daí, inclusive para ele.
