...
| Router + guarda de rota | Navegação entre as telas, com guarda de rota autenticada | Vue Router; integração OIDC via Keycloak com biblioteca cliente (ex. `keycloak-js` ou `oidc-client-ts`) |
...
| Keycloak | OIDC (cliente no browser) | Autenticação e obtenção de token para as chamadas à API |
...
- Biblioteca cliente OIDC (`keycloak-js` vs. `oidc-client-ts`) e fluxo de renovação de token.
...
| **Segurança** | Autorização por papel | Spring Security como *resource server* OAuth2/OIDC (Keycloak); papéis gestor de negócios, gerente comercial, RH, auditor, conforme os atores identificados no backlog. |
...
| Keycloak | OIDC (resource server) | Validação de token e papéis |
