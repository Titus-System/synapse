...
async def test_calculator_node_returns_the_model_response_as_the_new_message(
    scripted_model: ScriptedModel,
) -> None:
    scripted_model([AIMessage(content="the answer is 3")])

    update = await calculator_node({"messages": [HumanMessage(content="1 + 2?")]})
...
async def test_calculator_node_sends_the_system_prompt_first_then_the_history(
    scripted_model: ScriptedModel,
) -> None:
    model = scripted_model([AIMessage(content="ok")])
    question = HumanMessage(content="1 + 2?")

    await calculator_node({"messages": [question]})
