...
- **Never emits an audited number.** `codegen`'s output is rule *code* and *explanation text* — never a final currency value, percentage, or verdict presented as fact. If a node's output carries a computed simulation result instead of generated code or explanation, that's a bug: the arithmetic belongs to `worker`, over real data.
